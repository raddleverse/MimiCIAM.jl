@testmodule ComparisonData begin

    using MimiCIAM

    export comparison_outputdir
    ...
    # Generating the comparison output takes ~20 minutes, so it happens once here
    # rather than inside the test item, and lands in a temporary directory rather
    # than in the repository tree.
    comparison_outputdir = mktempdir(cleanup=true)

    # use the same random subset of segments that the validation data was written with
    write_MimiCIAM_comparison_files(comparison_outputdir, subset="random10.csv")
end
